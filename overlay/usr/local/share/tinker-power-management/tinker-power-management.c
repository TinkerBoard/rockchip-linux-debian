...
#define MAX_CPU_SQ 4
#define MAX_A55_FREQ 8

#define MAX_GPU_SQ 4
#define MAX_G52_FREQ 6
...
void draw_square_A(void) {
	...
	mvwprintw(BOARDA, 7, 3,"SoC/CPU\t= RK3568");
	...
	fp = popen("cat /proc/version | rev | awk {print'$1\" \"$2\" \"$3\" \"$4\" \"$5\" \"$6\" \"$7\" \"$8'} | rev", "r");
	memset(buffer, '\0', sizeof(buffer));
	fgets(buffer, sizeof(buffer), fp);
	pclose(fp);
...
void draw_square_B(int mode, int caller) {
	...
	fp = popen("cat /sys/class/devfreq/fde60000.gpu/governor", "r");
	memset(buffer, '\0', sizeof(buffer));
	fgets(buffer, sizeof(buffer), fp);
	pclose(fp);

	if (g52_min == 200 && g52_max == 600)
		gpu_dirty = 0;
	else
		gpu_dirty = 1;
...
void draw_square_FORMAC(int sq) {
	if (sq == 0) {
		...
		fp = popen("cat /proc/cpuinfo | grep 'CPU part' | grep -c '0xd05'", "r");
		memset(buffer, '\0', sizeof(buffer));
		fgets(buffer, sizeof(buffer), fp);
		pclose(fp);
		mvwprintw(FORMA[sq], 1, 2,"%d x Arm Cortex-A55", atoi(buffer));
		mvwprintw(FORMC[sq], 1, 2,"%d x Arm Cortex-A55", atoi(buffer));
	} else if (sq == 3) {
		...
		a55_min = atoi(buffer) / 1000;
		mvwprintw(FORMA[sq], 1, (SQFORM_WIDTH - 3) / 2,"%d ", a55_min);
		...
		a55_max = atoi(buffer) / 1000;
		mvwprintw(FORMA[sq], 1, (SQFORM_WIDTH - 4) / 2,"%d ", a55_max);
...
void draw_square_FORMBD(int sq) {
	if (sq == 0) {
		...
		fp = popen("cat /sys/class/devfreq/fde60000.gpu/device/gpuinfo | cut -d ' ' -f1", "r");
		memset(buffer, '\0', sizeof(buffer));
		fgets(buffer, sizeof(buffer), fp);
		pclose(fp);
		...
		fp = popen("cat /sys/class/devfreq/fde60000.gpu/min_freq", "r");
		memset(buffer, '\0', sizeof(buffer));
		fgets(buffer, sizeof(buffer), fp);
		pclose(fp);

		g52_min = atoi(buffer) / 1000000;
		mvwprintw(FORMB[sq], 1, (SQFORM_WIDTH - 3) / 2,"%d", g52_min);
		...
		fp = popen("cat /sys/class/devfreq/fde60000.gpu/max_freq", "r");
		memset(buffer, '\0', sizeof(buffer));
		fgets(buffer, sizeof(buffer), fp);
		pclose(fp);

		g52_max = atoi(buffer) / 1000000;
		mvwprintw(FORMB[sq], 1, (SQFORM_WIDTH - 3) / 2,"%d", g52_max);
...
void draw_square_A55FREQ(int sq, int mode) {
	if (sq == 0) {
		if (mode)
			mvwprintw(A55FREQ[sq], 1, 1, " 408");
		else
			mvwprintw(A55FREQ[sq], 1, 1, "    ");
	} else if (sq == 1) {
		if (mode)
			mvwprintw(A55FREQ[sq], 1, 1, " 600");
		else
			mvwprintw(A55FREQ[sq], 1, 1, "    ");
	} else if (sq == 2) {
		if (mode)
			mvwprintw(A55FREQ[sq], 1, 1, " 816");
		else
			mvwprintw(A55FREQ[sq], 1, 1, "    ");
	} else if (sq == 3) {
		if (mode)
			mvwprintw(A55FREQ[sq], 1, 1, "1104");
		else
			mvwprintw(A55FREQ[sq], 1, 1, "    ");
	} else if (sq == 4) {
		if (mode)
			mvwprintw(A55FREQ[sq], 1, 1, "1416");
		else
			mvwprintw(A55FREQ[sq], 1, 1, "    ");
	} else if (sq == 5) {
		if (mode)
			mvwprintw(A55FREQ[sq], 1, 1, "1608");
		else
			mvwprintw(A55FREQ[sq], 1, 1, "    ");
	} else if (sq == 6) {
		if (mode)
			mvwprintw(A55FREQ[sq], 1, 1, "1800");
		else
			mvwprintw(A55FREQ[sq], 1, 1, "    ");
	} else if (sq == 7) {
		if (mode)
			mvwprintw(A55FREQ[sq], 1, 1, "1992");
		else
			mvwprintw(A55FREQ[sq], 1, 1, "    ");
	}
}

void draw_square_G52FREQ(int sq, int mode) {
	if (sq == 0) {
		if (mode)
			mvwprintw(G52FREQ[sq], 1, 1, "200");
		else
			mvwprintw(G52FREQ[sq], 1, 1, "   ");
	} else if (sq == 1) {
		if (mode)
			mvwprintw(G52FREQ[sq], 1, 1, "300");
		else
			mvwprintw(G52FREQ[sq], 1, 1, "   ");
	} else if (sq == 2) {
		if (mode)
			mvwprintw(G52FREQ[sq], 1, 1, "400");
		else
			mvwprintw(G52FREQ[sq], 1, 1, "   ");
	} else if (sq == 3) {
		if (mode)
			mvwprintw(G52FREQ[sq], 1, 1, "600");
		else
			mvwprintw(G52FREQ[sq], 1, 1, "   ");
	} else if (sq == 4) {
		if (mode)
			mvwprintw(G52FREQ[sq], 1, 1, "700");
		else
			mvwprintw(G52FREQ[sq], 1, 1, "   ");
	} else if (sq == 5) {
		if (mode)
			mvwprintw(G52FREQ[sq], 1, 1, "800");
		else
			mvwprintw(G52FREQ[sq], 1, 1, "   ");
	}
}

void highlight_a55_freq(int sq) {
        wattron(A55FREQ[sq], A_BOLD);
        draw_square_A55FREQ(sq, cpu_mode);
        wattroff(A55FREQ[sq], A_BOLD);
}

void highlight_g52_freq(int sq) {
	wattron(G52FREQ[sq], A_BOLD);
	draw_square_G52FREQ(sq, gpu_mode);
	wattroff(G52FREQ[sq], A_BOLD);
}
...
void create_board(void) {
	...
	for (i = 0; i < 8; i++)		// sizeof(" xxxx ")
		A55FREQ[i] = derwin(BOARDB, 3, 6, SUBB_SHIFT_D + 4, SUB_SHIFT_R + i * 6 - i);

	for (i = 0; i < 6; i++)			// sizeof(" xxx ")
		G52FREQ[i] = derwin(BOARDB, 3, 5, SUBB_SHIFT_D + 12, SUB_SHIFT_R + i * 5 - i);
...
void gpu_info(void) {
	...
	fp = popen("cat /sys/class/devfreq/fde60000.gpu/cur_freq", "r");
	memset(buffer, '\0', sizeof(buffer));
	fgets(buffer, sizeof(buffer), fp);
	pclose(fp);
	...
	fp = popen("cat /sys/devices/platform/fde60000.gpu/utilisation", "r");
	memset(buffer, '\0', sizeof(buffer));
	fgets(buffer, sizeof(buffer), fp);
	pclose(fp);
			...
			if (cpu_mode == 1) {
				if (key == ' ') {
					if (switch_tag != 1 && cpu_dirty == 1) {
						...
						fp = popen("sudo sed -i 's/\\(a55_minfreq=\\).*/\\1408000/g' /boot/config.txt", "r");
						pclose(fp);

						fp = popen("sudo su -c \"echo 1992000 > /sys/devices/system/cpu/cpufreq/policy0/scaling_max_freq\"", "r");
						pclose(fp);

						fp = popen("sudo sed -i 's/\\(a55_maxfreq=\\).*/\\11992000/g' /boot/config.txt", "r");
						pclose(fp);
					...
					switch(switch_tag) {
						case 0:
							if (cpu_mode == 2 && a55_max >= 408 ) {
								...
								cpu_dirty = 0;
							} else if (cpu_mode == 3 && a55_min <= 408) {
								fp = popen("sudo su -c \"echo 408000 > /sys/devices/system/cpu/cpufreq/policy0/scaling_max_freq\"", "r");
								pclose(fp);

								fp = popen("sudo sed -i 's/\\(a55_maxfreq=\\).*/\\1408000/g' /boot/config.txt", "r");
								pclose(fp);
							}
							break;
						case 1:
							if (cpu_mode == 2 && a55_max >= 600 ) {
								fp = popen("sudo su -c \"echo 600000 > /sys/devices/system/cpu/cpufreq/policy0/scaling_min_freq\"", "r");
								pclose(fp);

								fp = popen("sudo sed -i 's/\\(a55_minfreq=\\).*/\\1600000/g' /boot/config.txt", "r");
								pclose(fp);
							} else if (cpu_mode == 3 && a55_min <= 600) {
								fp = popen("sudo su -c \"echo 600000 > /sys/devices/system/cpu/cpufreq/policy0/scaling_max_freq\"", "r");
								pclose(fp);

								fp = popen("sudo sed -i 's/\\(a55_maxfreq=\\).*/\\1600000/g' /boot/config.txt", "r");
								pclose(fp);
							}
							break;
						case 2:
							if (cpu_mode == 2 && a55_max >= 816 ) {
								fp = popen("sudo su -c \"echo 816000 > /sys/devices/system/cpu/cpufreq/policy0/scaling_min_freq\"", "r");
								pclose(fp);

								fp = popen("sudo sed -i 's/\\(a55_minfreq=\\).*/\\1816000/g' /boot/config.txt", "r");
								pclose(fp);
							} else if (cpu_mode == 3 && a55_min <= 816) {
								fp = popen("sudo su -c \"echo 816000 > /sys/devices/system/cpu/cpufreq/policy0/scaling_max_freq\"", "r");
								pclose(fp);

								fp = popen("sudo sed -i 's/\\(a55_maxfreq=\\).*/\\1816000/g' /boot/config.txt", "r");
								pclose(fp);
							}
							break;
						case 3:
							if (cpu_mode == 2 && a55_max >= 1104 ) {
								fp = popen("sudo su -c \"echo 1104000 > /sys/devices/system/cpu/cpufreq/policy0/scaling_min_freq\"", "r");
								pclose(fp);

								fp = popen("sudo sed -i 's/\\(a55_minfreq=\\).*/\\11104000/g' /boot/config.txt", "r");
								pclose(fp);
							} else if (cpu_mode == 3 && a55_min <= 1104) {
								fp = popen("sudo su -c \"echo 1104000 > /sys/devices/system/cpu/cpufreq/policy0/scaling_max_freq\"", "r");
								pclose(fp);

								fp = popen("sudo sed -i 's/\\(a55_maxfreq=\\).*/\\11104000/g' /boot/config.txt", "r");
								pclose(fp);
							}
							break;
						case 4:
							if (cpu_mode == 2 && a55_max >= 1416 ) {
								fp = popen("sudo su -c \"echo 1416000 > /sys/devices/system/cpu/cpufreq/policy0/scaling_min_freq\"", "r");
								pclose(fp);

								fp = popen("sudo sed -i 's/\\(a55_minfreq=\\).*/\\11416000/g' /boot/config.txt", "r");
								pclose(fp);
							} else if (cpu_mode == 3 && a55_min <= 1416) {
								fp = popen("sudo su -c \"echo 1416000 > /sys/devices/system/cpu/cpufreq/policy0/scaling_max_freq\"", "r");
								pclose(fp);

								fp = popen("sudo sed -i 's/\\(a55_maxfreq=\\).*/\\11416000/g' /boot/config.txt", "r");
								pclose(fp);
							}
							break;
						case 5:
							if (cpu_mode == 2 && a55_max >= 1608 ) {
								fp = popen("sudo su -c \"echo 1608000 > /sys/devices/system/cpu/cpufreq/policy0/scaling_min_freq\"", "r");
								pclose(fp);

								fp = popen("sudo sed -i 's/\\(a55_minfreq=\\).*/\\11608000/g' /boot/config.txt", "r");
								pclose(fp);
							} else if (cpu_mode == 3 && a55_min <= 1608) {
								fp = popen("sudo su -c \"echo 1608000 > /sys/devices/system/cpu/cpufreq/policy0/scaling_max_freq\"", "r");
								pclose(fp);

								fp = popen("sudo sed -i 's/\\(a55_maxfreq=\\).*/\\11608000/g' /boot/config.txt", "r");
								pclose(fp);
							}
							break;
						case 6:
							if (cpu_mode == 2 && a55_max >= 1800 ) {
								fp = popen("sudo su -c \"echo 1800000 > /sys/devices/system/cpu/cpufreq/policy0/scaling_min_freq\"", "r");
								pclose(fp);

								fp = popen("sudo sed -i 's/\\(a55_minfreq=\\).*/\\11800000/g' /boot/config.txt", "r");
								pclose(fp);
							} else if (cpu_mode == 3 && a55_min <= 1800) {
								fp = popen("sudo su -c \"echo 1800000 > /sys/devices/system/cpu/cpufreq/policy0/scaling_max_freq\"", "r");
								pclose(fp);

								fp = popen("sudo sed -i 's/\\(a55_maxfreq=\\).*/\\11800000/g' /boot/config.txt", "r");
								pclose(fp);
							}
							break;
						case 7:
							if (cpu_mode == 2 && a55_max >= 1992 ) {
								fp = popen("sudo su -c \"echo 1992000 > /sys/devices/system/cpu/cpufreq/policy0/scaling_min_freq\"", "r");
								pclose(fp);

								fp = popen("sudo sed -i 's/\\(a55_minfreq=\\).*/\\11992000/g' /boot/config.txt", "r");
								pclose(fp);
							} else if (cpu_mode == 3 && a55_min <= 1992) {
								fp = popen("sudo su -c \"echo 1992000 > /sys/devices/system/cpu/cpufreq/policy0/scaling_max_freq\"", "r");
								pclose(fp);
				...
				if (key == 'q' || key == ' ') {
					...
					for (i = 0; i < MAX_A55_FREQ; i++)
						draw_square_A55FREQ(i, cpu_mode);
					highlight_menu(curr_cpu_menu, 0);
				} else if (key == 'C') {        //right
					draw_square_A55FREQ(switch_tag, cpu_mode);

					if (switch_tag < MAX_A55_FREQ - 1)
						highlight_a55_freq(++switch_tag);
					else {
						switch_tag = 0;
						highlight_a55_freq(switch_tag);
					}
					...
					if (curr_cpu_governor == 1 || curr_gpu_governor == 1) {
						if (priority) {
							...
							switch(curr_gpu_menu) {
								case 1:
								case 2:
									for (i = 0; i < MAX_G52_FREQ; i++)
										draw_square_G52FREQ(i, gpu_mode);

									highlight_g52_freq(curr_tag);
									break;
							}
				...
				if (key == ' ') {
					if (switch_tag != 1 && gpu_dirty == 1) {
						fp = popen("sudo su -c \"echo 200000000 > /sys/class/devfreq/fde60000.gpu/min_freq\"", "r");
						pclose(fp);

						fp = popen("sudo sed -i 's/\\(g52_minfreq=\\).*/\\1200000000/g' /boot/config.txt", "r");
						pclose(fp);

						fp = popen("sudo su -c \"echo 800000000 > /sys/class/devfreq/fde60000.gpu/max_freq\"", "r");
						pclose(fp);

						fp = popen("sudo sed -i 's/\\(g52_maxfreq=\\).*/\\1800000000/g' /boot/config.txt", "r");
						pclose(fp);
					...
					switch(switch_tag) {
						case 0:
							...
							fp = popen("sudo su -c \"echo simple_ondemand > /sys/class/devfreq/fde60000.gpu/governor\"", "r");
							pclose(fp);
						...
						case 2:
							...
							fp = popen("sudo su -c \"echo powersave > /sys/class/devfreq/fde60000.gpu/governor\"", "r");
							pclose(fp);
						...
						case 3:
							...
							fp = popen("sudo su -c \"echo performance > /sys/class/devfreq/fde60000.gpu/governor\"", "r");
							pclose(fp);
					...
					switch(switch_tag) {
						case 0:
							if (gpu_mode == 2 && g52_max >= 200 ) {
								...
								gpu_dirty = 0;
							} else if (gpu_mode == 3 && g52_min <= 200) {
								fp = popen("sudo su -c \"echo 200000000 > /sys/class/devfreq/fde60000.gpu/max_freq\"", "r");
								pclose(fp);

								fp = popen("sudo sed -i 's/\\(g52_maxfreq=\\).*/\\1200000000/g' /boot/config.txt", "r");
								pclose(fp);
							}
							break;
						case 1:
							if (gpu_mode == 2 && g52_max >= 300 ) {
								fp = popen("sudo su -c \"echo 300000000 > /sys/class/devfreq/fde60000.gpu/min_freq\"", "r");
								pclose(fp);

								fp = popen("sudo sed -i 's/\\(g52_minfreq=\\).*/\\1300000000/g' /boot/config.txt", "r");
								pclose(fp);
							} else if (gpu_mode == 3 && g52_min <= 300) {
								fp = popen("sudo su -c \"echo 300000000 > /sys/class/devfreq/fde60000.gpu/max_freq\"", "r");
								pclose(fp);

								fp = popen("sudo sed -i 's/\\(g52_maxfreq=\\).*/\\1300000000/g' /boot/config.txt", "r");
								pclose(fp);
							}
							break;
						case 2:
							if (gpu_mode == 2 && g52_max >= 400 ) {
								fp = popen("sudo su -c \"echo 400000000 > /sys/class/devfreq/fde60000.gpu/min_freq\"", "r");
								pclose(fp);

								fp = popen("sudo sed -i 's/\\(g52_minfreq=\\).*/\\1400000000/g' /boot/config.txt", "r");
								pclose(fp);
							} else if (gpu_mode == 3 && g52_min <= 400) {
								fp = popen("sudo su -c \"echo 400000000 > /sys/class/devfreq/fde60000.gpu/max_freq\"", "r");
								pclose(fp);

								fp = popen("sudo sed -i 's/\\(g52_maxfreq=\\).*/\\1400000000/g' /boot/config.txt", "r");
								pclose(fp);
							}
							break;
						case 3:
							if (gpu_mode == 2 && g52_max >= 600 ) {
								fp = popen("sudo su -c \"echo 600000000 > /sys/class/devfreq/fde60000.gpu/min_freq\"", "r");
								pclose(fp);

								fp = popen("sudo sed -i 's/\\(g52_minfreq=\\).*/\\1600000000/g' /boot/config.txt", "r");
								pclose(fp);
							} else if (gpu_mode == 3 && g52_min <= 600) {
								fp = popen("sudo su -c \"echo 600000000 > /sys/class/devfreq/fde60000.gpu/max_freq\"", "r");
								pclose(fp);

								fp = popen("sudo sed -i 's/\\(g52_maxfreq=\\).*/\\1600000000/g' /boot/config.txt", "r");
								pclose(fp);
							}
							break;
						case 4:
							if (gpu_mode == 2 && g52_max >= 700 ) {
								fp = popen("sudo su -c \"echo 700000000 > /sys/class/devfreq/fde60000.gpu/min_freq\"", "r");
								pclose(fp);

								fp = popen("sudo sed -i 's/\\(g52_minfreq=\\).*/\\1700000000/g' /boot/config.txt", "r");
								pclose(fp);
							} else if (gpu_mode == 3 && g52_min <= 700) {
								fp = popen("sudo su -c \"echo 700000000 > /sys/class/devfreq/fde60000.gpu/max_freq\"", "r");
								pclose(fp);

								fp = popen("sudo sed -i 's/\\(g52_maxfreq=\\).*/\\1700000000/g' /boot/config.txt", "r");
								pclose(fp);
							}
							break;
						case 5:
							if (gpu_mode == 2 && g52_max >= 800 ) {
								fp = popen("sudo su -c \"echo 800000000 > /sys/class/devfreq/fde60000.gpu/min_freq\"", "r");
								pclose(fp);

								fp = popen("sudo sed -i 's/\\(g52_minfreq=\\).*/\\1800000000/g' /boot/config.txt", "r");
								pclose(fp);
							} else if (gpu_mode == 3 && g52_min <= 800) {
								fp = popen("sudo su -c \"echo 800000000 > /sys/class/devfreq/fde60000.gpu/max_freq\"", "r");
								pclose(fp);
				...
				if (key == 'q' || key == ' ') {
					...
					draw_square_G52FREQ(switch_tag, gpu_mode);

					if (switch_tag < MAX_G52_FREQ - 1)
						highlight_g52_freq(++switch_tag);
					else {
						switch_tag = 0;
						highlight_g52_freq(switch_tag);
					}
