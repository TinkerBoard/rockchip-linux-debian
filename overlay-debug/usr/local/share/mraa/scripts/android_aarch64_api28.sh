...
PKG_CONFIG_LIBDIR="$NDK_HOME/platforms/android-28/arch-arm64/usr/lib/"
...
cmake \
  ...
  -DCMAKE_FIND_ROOT_PATH=$NDK_HOME/platforms/android-28/arch-arm64/ \
...
cp $NDK_HOME/sources/cxx-stl/llvm-libc++/libs/arm64-v8a/libc++_shared.so build/src/
...
mkdir -p $AAR_DIRECTORY/jni/arm64-v8a
...
# Copy over the so's
cp src/*.so $AAR_DIRECTORY/jni/arm64-v8a
cp src/java/*.so $AAR_DIRECTORY/jni/arm64-v8a
