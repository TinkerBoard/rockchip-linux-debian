...
static void checkDevTree (char *argv [])
{
	...
	#ifdef TINKER_BOARD
	fprintf (stderr,
	"%s: Unable to load/unload modules as this TINKER has the device tree enabled.\n"
	"  You need to run the tinker-config program (as root) and select the\n"
...
static void doI2Cdetect (int argc, char *argv [])
{
  ...
  if(argv[2] == NULL) {
	...
	sprintf(command, "%s -y %d", I2CDETECT, port);
  	...
  	sprintf (command, "%s -y %s", I2CDETECT, argv[2]) ;
  }
  #endif
  DEBUG("doI2Cdetect command=%s\n", command);
  if (system (command) < 0)
...
static void doPwmClock (int argc, char *argv [])
{
  ...
  #ifdef TINKER_BOARD
  if ((clock < 1) || (clock > 255))
  {
    fprintf (stderr, "%s: clock must be between 0 and 256\n", argv [0]) ;
    exit (1) ;
  }
  #else
