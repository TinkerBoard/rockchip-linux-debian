...
script_version="3.14.20220125-tegra"
...
function TTFF(){
	...
	if [[ $GPSMODULE == 'ublox' ]]; then
		...
		stopGPS >> $LogFile2
		send_at_command "at+qgpsdel=0" >> $LogFile2
		read_at_result "at+qgpsdel=0" >> $LogFile2
		startGPS >> $LogFile2
	fi
   	...
   	busybox microcom -t 2000 -s $baudrate $GPSPORT | while read -r line; do
		...
		if [[ $NMEAHead == '$GNRMC' ]] || [[ $NMEAHead == '$GPRMC' ]]; then
			...
			if [ "$data0" != "" ] && [ "$data1" != "" ]; then
				...
				echo "TTFF:$count pos0=[$data0], pos1=[$data1]\n" >> $LogFile2
				exit
			fi
		fi
	done
...
function getSVwithIQXtream(){
	...
	if [[ $GPSMODULE == 'ublox' ]]; then
		...
		if [ -e $GPSPORT ]; then
			...
			stopGPS >> $LogFile1
			send_at_command "at+qgpsdel=0" >> $LogFile1
			read_at_result "at+qgpsdel=0" >> $LogFile1
			startGPS >> $LogFile1
			sleep 2
			...
			#microcom -t 2000 -s $baudrate $GPSPORT | while read -r line; do
				...
				if [[ $NMEAHead == '$GPGSV' ]]; then
					...
					SVcn1=`echo "$SVcn1"  | awk -F "*" '{print $1}'`
					if $def_debug ; then
						echo "[$spec_count, $value_count, SVid:$SVid1]Processing $line" 
					fi
					if [ "$SVid1" == "01" ] && [ "$SVcn1" != "" ]; then
						value=$((value+SVcn1))
						((++value_count))
...
function CheckModule(){
	...
	retval=$?
	if [ "$retval" == 1 ]; then
		echo "PASS" | tee -a $LogFile3
	else
		echo "FAIL" | tee -a $LogFile3
	fi
}
...
function help(){
	...
	echo "------------------------------------------------------------------"
	echo "Factory Test:"
	echo "	0 0(1,2): check ublox(locosys,quectel) module"
	echo "	1 0(1) 10: Get GPS signal for 10 times"
	echo "	2 0(1) 60: Test TTFF 60 sec"
	echo "	3 0(1,2) 10 40: Get ublox(locosys,quectel) signal 10"
	echo "	                times and spec is :40  (For IQXtreams)"
	echo "	4 0(1): Get ublox(locosys) FW version"
	echo "------------------------------------------------------------------"
	echo "Fucntion Test:"
	echo "	calibration /dev/ttymxc3 locosys (ublox)"
	echo "	info        /dev/ttymxc3 locosys (ublox ,locosys)"
	echo "	coldstart   /dev/ttymxc3 locosys (ublox ,locosys)"
	echo "	distance    /dev/ttymxc3 locosys (ublox ,locosys) lat1 long1"
...
case "$1" in
	...
	'hw_reset')
		...
		echo -ne $UBLOX_HW_RESET >  $GPSPORT
			...
			read_at_result "at+cfun=1,1"
			if [[ $at_result == *"OK"* ]]; then
				echo "reset quectel module success"
			fi
		fi
	;;
	'enableUBX')
