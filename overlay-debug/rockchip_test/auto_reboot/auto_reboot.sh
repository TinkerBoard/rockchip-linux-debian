...
if [ ! -e "/data/rockchip_test/auto_reboot.sh" ]; then
	cp -f /rockchip_test/auto_reboot/auto_reboot.sh /data/rockchip_test/
	cp -f /rockchip_test/auto_reboot/auto_reboot_test.sh /etc/init.d/
	cp -f /rockchip_test/auto_reboot/rockchip_reboot.service /lib/systemd/system/
	ln -sf /lib/systemd/system/rockchip_reboot.service /etc/systemd/system/multi-user.target.wants/

        echo $total > /data/rockchip_test/reboot_total_cnt
    sync
fi
...
then
    ...
    rm -rf /data/rockchip_test/reboot_total_cnt
    rm -f $CNT
    sync
    exit 0
fi
